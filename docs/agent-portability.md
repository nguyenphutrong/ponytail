...
| Amp | `.amp/plugins/ponytail/`, `AGENTS.md` | Native directory plugin: injects the mode-filtered ruleset on every turn, keeps `lite/full/ultra/off` state per thread, adds a command-palette mode selector, and registers all six bundled skills as `ponytail:<skill>`. `AGENTS.md` remains the zero-setup instruction-tier fallback. |
